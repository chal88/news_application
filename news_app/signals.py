...
def notify_on_article_approval(sender, instance, created, **kwargs):
    """
    Notify all readers when an article is approved.
    """
    ...
    email_list = [user.email for user in readers if user.email]
...
def post_to_x(article):
    """
    Posts article summary to X (formerly Twitter) using Tweepy.
    OAuth credentials must be in settings.py.
    Logs exceptions if posting fails.
    """
    if tweepy is None:
    ...
    try:
        ...
        logger.info(f"Article '{article.title}' successfully posted to X")

    except Exception:
        logger.exception(f"Failed to post article '{article.title}' to X")
